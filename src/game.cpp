...
void game::show_intro()
{
    cout << colored_string("#" + center_string(mStrings[GAME_TITLE]) + "#", colored_string::PrintColors::GREEN) << endl
         << colored_string("#" + center_string(mStrings[ORIGINAL_AUTHOR]) + "#", colored_string::PrintColors::GREEN)
         << endl
...
unsigned game::show_menu()
{
    ...
    int option = get_value_in_range(1u, static_cast<unsigned>(MENU_EXIT - MENU_FIRST_OPTION + 1u),
                                    mStrings[MENU_INPUT_PROMPT], mStrings[ERROR_STRING]);
...
void game::change_language()
{
    ...
    for (unsigned i = 0;
         i < sizeof(mLanguages) / sizeof(mLanguages[static_cast<languages::AvailableLanguages>(0)]); i++)
        cout << i + 1 << ". " << mLanguages[static_cast<languages::AvailableLanguages>(i)].name << endl;

    int option = get_value_in_range(1u, static_cast<unsigned>(languages::NUMBER_OF_AVAILABLE_LANGUAGES),
                                    mStrings[MENU_INPUT_PROMPT], mStrings[ERROR_STRING]);
